...
function getFromPaperMc() {
  ...
  if [[ ${buildId^^} = LATEST ]]; then
    if ! buildId=$(get --json-path=".builds[0]" "https://fill.papermc.io/v3/projects/${project}/versions/${version}"); then
  ...
  if ! jar=$(get --json-path=".downloads.server:default.url" "https://fill.papermc.io/v3/projects/${project}/versions/${version}/builds/${buildId}"); then
...
wget -O /download/protocolize.jar https://exceptionflug.de/protocolize/protocolize-velocity-2.4.3.jar
